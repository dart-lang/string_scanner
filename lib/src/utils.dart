...
/// The lower bound of Unicode's supplementary plane.
const _supplementaryPlaneLowerBound = 0x10000;

/// The lower bound of the UTF-16 high surrogate block.
const _highSurrogateLowerBound = 0xD800;

/// The lower bound of the UTF-16 low surrogate block.
...
/// A bit mask that covers the lower bits of a code point, which are then
/// encoded into its low surrogate.
const _lowSurrogateMask = (1 << _surrogateBits) - 1;
...
bool isHighSurrogate(int codeUnit) =>
    (codeUnit >> _surrogateBits) ==
    (_highSurrogateLowerBound >> _surrogateBits);
...
int lowSurrogate(int codePoint) {
  assert(inSupplementaryPlane(codePoint));
  return ((codePoint - _supplementaryPlaneLowerBound) & _lowSurrogateMask) +
...
int decodeSurrogatePair(int highSurrogate, int lowSurrogate) {
  ...
  return _supplementaryPlaneLowerBound +
      ((highSurrogate - _highSurrogateLowerBound) << _surrogateBits) +
      lowSurrogate -
      _lowSurrogateLowerBound;
}
